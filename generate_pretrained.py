...
========================================================
Run ONCE to create pretrained_base.pth — a 36-class CNN (A-Z + 0-9)
trained on synthetic data. This is the Cognex-style base that already
knows all characters before it ever sees your real images.
...
def main():
    ...
    print(f"[INFO] Device: {device} | {N_CLASSES} chars × {args.samples} = {N_CLASSES*args.samples} images", flush=True)

    data = []
    for ci, ch in enumerate(FULL_CHARS):
        for _ in range(args.samples):
            data.append((augment(render_char(ch)), ci))
        if (ci+1) % 9 == 0: print(f"  Generated {ci+1}/{N_CLASSES}...", flush=True)
    random.shuffle(data)
